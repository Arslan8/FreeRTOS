...
COMPAT
ptr<RCC_TypeDef> get_clock_controller(void);


__attribute__((nonnull))
COMPAT ptr<RCC_TypeDef> get_clock_controller(void) {
    unchecked {
        RCC = _Assume_bounds_cast<RCC_TypeDef *>(RCC_BASE);
		return RCC;
    }
}
void clock_init() {
  ptr<RCC_TypeDef> RCC = get_clock_controller();
  if (RCC) {
  /* Set HSION bit */
